...
func (o *DeleteServiceServiceIDURL) Build() (*url.URL, error) {
	...
	var _path = "/service/{service_id}"

	serviceID := o.ServiceID.String()
	if serviceID != "" {
		_path = strings.ReplaceAll(_path, "{service_id}", serviceID)
	} else {
		return nil, errors.New("serviceId is required on DeleteServiceServiceIDURL")
	}
...
func (o *DeleteServiceServiceIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on DeleteServiceServiceIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on DeleteServiceServiceIDURL")
	}
