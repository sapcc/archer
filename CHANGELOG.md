...
- archer-f5-agent: `ProcessEndpoint` defers its AS3 post until the owning service is `AVAILABLE`, avoiding a 422 for the not-yet-created `Common/Shared` snatpool when an endpoint run races ahead of the service during migration.
- archer-f5-agent, archer-ni-agent: `ProcessServices` and `ProcessEndpoint` status writes are now guarded on the status the run acted on (via shared `db` helpers), so a delete (or other API change) arriving mid-reconcile is no longer clobbered back to `AVAILABLE` — the change survives and reconciles on the next notification.
- archer-f5-agent: a `ProcessServices` run skipped because another holds the serialization lock now reschedules after a short delay instead of logging an error-level `Job FAILED` or waiting for the next sync interval, so a migration whose notification loses the lock race still converges within seconds.
- archer-f5-agent: `ProcessServices` enqueues are coalesced — a burst of service notifications collapses to at most one queued run (with one guaranteed follow-up for signals arriving mid-run) instead of queueing one full reconcile per notification.
- archer-f5-agent: an unexpected error resolving one service no longer aborts a `ProcessServices` run — the `Common` tenant is still re-posted (pruning migrated-away services) and healthy services' statuses are persisted before the error is returned for retry.
- archer-f5-agent: the F5OS token is refreshed 60s before its JWT expiry and its access is mutex-guarded, removing recurring 401s and re-auth backoff that slowed VCMP reconciles.
- archer-f5-agent: `ProcessServices` and `ProcessEndpoint` no longer hold `FOR UPDATE OF service` / `FOR UPDATE OF endpoint` row locks across the BigIP/Neutron I/O of a reconcile cycle. The agent now serializes its own runs with a transaction-scoped advisory lock (which carries no `service`/`endpoint` row locks), reads the rows without `FOR UPDATE`, performs all external I/O with no open write transaction, and persists status transitions in a short, status-guarded final transaction. This stops the agent — which can stay `idle in transaction` for seconds while waiting on BigIP/Neutron — from blocking the API's `FOR UPDATE` handlers into a `lock_timeout` and spurious HTTP 503 on `DELETE /service`.
...
[Unreleased]: https://github.com/sapcc/archer/compare/v2.7.2...HEAD

[2.7.2]: https://github.com/sapcc/archer/compare/v2.7.1...v2.7.2
