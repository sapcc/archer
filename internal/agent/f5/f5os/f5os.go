...
func (f *F5OS) EnsureInterfaceVlan(_ int) error {
	// noop on F5OS
	return nil
}
...
func (f *F5OS) DeleteVLAN(name string) error {
	path := fmt.Sprintf("api/data/openconfig-vlan:vlans/vlan=%s", name)
	req := f.newRequest("DELETE", path, nil)
	if err := f.apiCall(req, nil); err != nil {
		return fmt.Errorf("error deleting VLAN %s on %s: %w", name, f.uri.Host, err)
	}
	return nil
}
