...
build-all: build/archer-server build/archer-f5-agent build/archerctl build/archer-migrate build/archer-ni-agent

build/archer-server: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=archer-server -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -o build/archer-server ./cmd/archer-server

build/archer-f5-agent: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=archer-f5-agent -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -o build/archer-f5-agent ./cmd/archer-f5-agent

build/archerctl: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=archerctl -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -o build/archerctl ./cmd/archerctl

build/archer-migrate: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=archer-migrate -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -o build/archer-migrate ./cmd/archer-migrate

build/archer-ni-agent: FORCE
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=archer-ni-agent -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -o build/archer-ni-agent ./cmd/archer-ni-agent
...
install: FORCE build/archer-server build/archer-f5-agent build/archerctl build/archer-migrate build/archer-ni-agent
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/archer-server "$(DESTDIR)$(PREFIX)/bin/archer-server"
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/archer-f5-agent "$(DESTDIR)$(PREFIX)/bin/archer-f5-agent"
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/archerctl "$(DESTDIR)$(PREFIX)/bin/archerctl"
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/archer-migrate "$(DESTDIR)$(PREFIX)/bin/archer-migrate"
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/archer-ni-agent "$(DESTDIR)$(PREFIX)/bin/archer-ni-agent"
...
build/cover.out: FORCE | build
	...
	@env $(GO_TESTENV) go test -shuffle=on -p 1 -coverprofile=build/coverprofile.out $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=archer -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -covermode=count -coverpkg=$(subst $(space),$(comma),$(GO_COVERPKGS)) $(GO_TESTFLAGS) $(GO_TESTPKGS)
...
goimports: FORCE install-goimports
	@printf "\e[1;36m>> goimports -w -local https://github.com/sapcc/archer\e[0m\n"
	@goimports -w -local github.com/sapcc/archer $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/archer-server\e[0m          Build archer-server.\n"
	@printf "  \e[36mbuild/archer-f5-agent\e[0m        Build archer-f5-agent.\n"
	@printf "  \e[36mbuild/archerctl\e[0m              Build archerctl.\n"
	@printf "  \e[36mbuild/archer-migrate\e[0m         Build archer-migrate.\n"
	@printf "  \e[36mbuild/archer-ni-agent\e[0m        Build archer-ni-agent.\n"
